...
#include "../header/ISoundFileWrapper.h"
...
SoundObject::SoundObject(const MyStringAnsi & fileName, const MyStringAnsi & name)
{
	...
	this->settings.pos = MyMath::Vector3(0, 0, 0);
	this->settings.velocity = MyMath::Vector3(0, 0, 0);
...
SoundObject::SoundObject(char * rawData, uint32 dataSize, SoundInfo soundInfo, const MyStringAnsi & name)
{
	this->SINGLE_BUFFER_SIZE = 64 * 1024;
...
void SoundObject::GetRawData(std::vector<char> * rawData)
{
	...
	{
		MyUtils::Logger::LogWarning("Can not get raw data for sound (%s), that is in PLAY state.", this->settings.name.GetConstString());
		return;
	}
...
void SoundObject::LoadData()
{
	...
	VFS_FILE * vfsFile = VFS::GetInstance()->OpenFile(this->settings.fileName);

    if (vfsFile == NULL)
    {
        MyUtils::Logger::LogError("File %s not found", this->settings.fileName.GetConstString());
        return;
    }
	...
	{
		MyUtils::Logger::LogError("File extension %s not supported.", vfsFile->ext);
	}
